...
from bench.metrics import cost_null_reason, graph_cleanliness, speed_metrics
...
def run_kg(corpus_dir, project_dir, *, scale, embedder_opt="fake") -> RunResult:
    ...
    (paths.root / "extracted.json").write_text(json.dumps({"nodes": nodes, "edges": edges}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    cfg = Config.from_path(paths.config)
    adapter = SQLiteAdapter(paths.kg_db)
    embedder = FakeEmbedder()
    gate = Gate(adapter, Resolver(adapter, embedder, cfg.thresholds), Deduper(adapter, embedder, cfg.thresholds), embedder, cfg, cfg.project.user_id)
